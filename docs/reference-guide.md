...
git clone https://github.com/ccc112233ccc/openurma-gem5-lab.git
cd openurma-gem5-lab
./lab --runtime docker setup --jobs 2
```
...
[`kvm-functional-mode.md`](kvm-functional-mode.md)。
...
`./lab --runtime docker setup` 是统一 CLI 的 Docker 构建入口，包装层完成以下工作：

1. 构建 Ubuntu 22.04 ARM64 工具容器；
2. 为 macOS 上的 OLK 源码提供大小写敏感卷；
3. 在容器里调用同一个 `lab --runtime native setup --skip-deps` 核心流程。
...
```bash
./lab --runtime docker setup --sources-only
```
...
等 `lab status` 显示两个 `guest shell ready` 后，完成双端控制网络初始化：
...
不是卡死。停止实验使用 `./lab stop`。更完整的 SEND/READ/WRITE 和包长扫描
命令见后文。
...
[`mooncake-urma-bringup.md`](mooncake-urma-bringup.md)。
...
copy exported by `lab build kernel`:
...
`OPENURMA_M5TERM_PORT=3457 ./lab attach 0`. Both processes run in the same
...
`lab start` starts two separate gem5 processes by default. Each process owns an OLK-6.6
...
[`ns3ub-network-boundary.md`](ns3ub-network-boundary.md).
...
For a reproducible size curve, use the sweep helper after `lab sync`:
...
`lab sync` also rejects a guest that did not register its architected timer.
...
docker exec -it openurma-gem5-lab bash -lc \
  'cd /workspace/openurma-gem5-lab && JOBS=1 ./lab --runtime native build gem5'
```
