    ...
    with tempfile.TemporaryDirectory(prefix="openurma-adapter-four-") as directory:
        ...
        process = subprocess.Popen(
            [str(binary), "--multi", "1", "100t", "50t", "400", "0",
             "", "1", "1,0,3,2", *(str(path) for path in paths)],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.PIPE,
            text=True,
        )
        try:
            # Endpoint 2 must observe endpoint 3's phase, not either member of
            # the unrelated 0<->1 pair.
            struct.pack_into("<Q", maps[3], 4096, 7)
            wait_for(
                lambda: struct.unpack_from("<Q", maps[2], 4104)[0] == 7,
                process,
                "four-endpoint switch did not mirror paired sync phase",
            )
            assert struct.unpack_from("<Q", maps[0], 4104)[0] == 0
            assert struct.unpack_from("<Q", maps[1], 4104)[0] == 0

            payload = bytes(range(64))
            transaction = bytes(40) + payload
            header = MESSAGE.pack(
                len(transaction), 1, 2000, 2100, 19, 0, 0, 2, 0, bytes(24)
            )
            offset = slot_offset(1, 0, 0)
            maps[2][offset : offset + MESSAGE.size] = header
            maps[2][offset + MESSAGE.size :
                    offset + MESSAGE.size + len(transaction)] = transaction
            struct.pack_into("<Q", maps[2], index_offset(1, 0), 1)
            wait_for(
                lambda: struct.unpack_from(
                    "<Q", maps[3], index_offset(0, 0))[0] == 1,
                process,
                "four-endpoint switch did not forward paired DATA",
            )
            assert struct.unpack_from("<Q", maps[0], index_offset(0, 0))[0] == 0
