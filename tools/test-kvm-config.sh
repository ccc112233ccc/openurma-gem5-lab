...
"$lab/lab" --runtime native start --profile kvm --print-config >"$tmp/kvm"
...
"$lab/lab" --runtime native start --profile kvm --sync \
...
"$lab/lab" --runtime native start --profile fast --no-sync \
...
OPENURMA_EXECUTION_MODE=native OPENURMA_UDMA_POLL_INTERVAL=25us \
    "$lab/lab" --runtime native start --profile kvm --print-config >"$tmp/kvm-explicit"
grep -qx 'udma_poll_interval=25us' "$tmp/kvm-explicit"

if "$lab/lab" --runtime native start --profile kvm \
...
"$lab/lab" --runtime native start --profile kvm \
